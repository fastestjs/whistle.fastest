...
function removeVProxy(requestUrl, pattern) {
    ...
    let pureRequestUrl = urlParse(requestUrl).path;

    // 替换之后的 r 为 /xxx/yyy/zzz.js
    let r = pureRequestUrl.replace(new RegExp(MATCH_PROXY_TAG + '/' + fastestUtil.replaceDotToUnderline(pattern) + '/' + MATCH_PROXY_TAG + '/', 'gi'), '');
