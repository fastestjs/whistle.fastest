...
function removeVProxy(requestUrl, pattern) {
    // 正常应该的 requestUrl 应该为 /xxx/yyy/zzz.js
    // 但有时候可能会有 http://domain.com/xxx/yyy/zzz.js
    var pureRequestUrl = urlParse(requestUrl).path;

    // 替换之后的 r 为 /xxx/yyy/zzz.js
    var r = pureRequestUrl.replace(new RegExp(MATCH_PROXY_TAG + '/' + fastestUtil.replaceDotToUnderline(pattern) + '/' + MATCH_PROXY_TAG + '/', 'gi'), '');
    ...
    // 加上域名 domain.com/xxx/yyy/zzz.js
    r = pattern + r;

    // 加上 http://domain.com/xxx/yyy/zzz.js
    if (r.indexOf('http') < 0) {
        r = 'http://' + r;
    }

    return urlParse(r).path;
}
...
function addVProxyForContent(content, pattern, proxyDomain) {
    var newContent = content.replace(new RegExp(pattern, 'gi'), proxyDomain + '/' + MATCH_PROXY_TAG + '/' + fastestUtil.replaceDotToUnderline(pattern) + '/' + MATCH_PROXY_TAG);
...
module.exports = {
    MATCH_PROXY_TAG: MATCH_PROXY_TAG,
    isMatchVProxy: isMatchVProxy,
    removeVProxy: removeVProxy,
