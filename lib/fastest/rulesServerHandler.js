            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    case 0:
                        ...
                        console.log('\n\n[rulesServerHandler.js] handleRequestRules(ctx) start', ctx.fullUrl);

                        // 获取请求参数
                        requestParams = getRequestParams(ctx);

                        console.log('[rulesServerHandler.js] handleRequestRules(ctx) requestParams', requestParams);
                        ...
                        ctx.rules = [].concat((0, _toConsumableArray3.default)(proxyEnv.formatRules()));
                        ...
                        console.log('[rulesServerHandler.js] handleRequestRules(ctx) ---ctx.rules---', ctx.rules);

                        // 将配置参数传递下去，后续流程需要用到这些配置
                        ctx.options.proxyEnv = proxyEnv;

                        console.log('[rulesServerHandler.js] handleRequestRules(ctx) end');
